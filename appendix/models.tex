...
\begin{table}[h] % Placement options: h (here), t (top), b (bottom), p (page)
    \input{table/upload_request_model.tex}
...
{Task document} obtained using \linkinternaltaskmanagement{this} Globus API 
described in Appendix \ref{appendix:globusapis}.

\begin{table}[h] % Placement options: h (here), t (top), b (bottom), p (page)
    \input{table/transfer_task_model.tex}
